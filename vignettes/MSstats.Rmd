...
* `n` : number of steps, which is the number of different sample sizes to simulate. Default is `5`.
* `step` : number of samples per condition to increase at each step. Default is `20`.
* `noise` : the fraction of protein variance to change at each step. Positive value indicates increasing noise in the data while negative value means decreasing noise. Default is `0.0`.
* `iter` : number of times to repeat simulation experiments. Default is `5`.
