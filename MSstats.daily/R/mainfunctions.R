...
dataProcessPlots<-function(data=data,type=type,featureName="Transition",ylimUp=FALSE,ylimDown=FALSE,scale=FALSE,interval="CI",x.axis.size=10,y.axis.size=10,text.size=4,text.angle=0,legend.size=7,dot.size=3,width=10,height=10, which.Protein="all", address=""){
  ...
  if (toupper(type)=="PROFILEPLOT"){
    ...
    for (i in 1:nlevels(data$PROTEIN)){	
      ...
      if(nrow(sub)==sum(is.na(sub$ABUNDANCE))){
        message(paste("Can't the Profile plot for ",unique(sub$PROTEIN), "(",i," of ",length(unique(data$PROTEIN)),") because all measurements are NAs."))
        next()
      }
  ...
  if (toupper(type)=="QCPLOT"){
    ...
    for (i in 1:nlevels(data$PROTEIN)){	
      ...
      ## if all measurements are NA,
      if(nrow(sub)==sum(is.na(sub$ABUNDANCE))){
        message(paste("Can't the Quality Control plot for ",unique(sub$PROTEIN), "(",i," of ",length(unique(data$PROTEIN)),") because all measurements are NAs."))
        next()
      }
  ...
  if(toupper(type)=="CONDITIONPLOT"){
    ...
    if(nlevels(data$LABEL)==1){
      for (i in 1:nlevels(data$PROTEIN)){	
        ...
        ## if all measurements are NA,
        if(nrow(sub)==sum(is.na(sub$ABUNDANCE))){
          message(paste("Can't the Condition plot for ",unique(sub$PROTEIN), "(",i," of ",length(unique(data$PROTEIN)),") because all measurements are NAs."))
          next()
        }
