...
quantification <- function(data, type="Sample", format="matrix") {
  ...
  processout <- rbind(processout,
                      as.matrix(c(" ", " ", "MSstats - quantification function", " "),
                                ncol=1))
  ...
  ## all input
  processout <- rbind(processout, c(paste0("type of quantification = ", type)))
  processout <- rbind(processout, c(paste0("format of output = ", format)))
  write.table(processout, file=finalfile, row.names=FALSE)
