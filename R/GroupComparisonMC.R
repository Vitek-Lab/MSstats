...
#' @export groupComparisonMC
#' @import lme4 
#' @import limma

groupComparisonMC <- function(contrast.matrix=contrast.matrix,
	...
	processall <- data$ProcessedData
  	...
  	processout_MC = c()
  	GC_results <- foreach(i=1: nlevels(rqall$Protein), .export =c(".checkSingleSubject", '.checkTechReplicate','.checkRunbyFeature','.checkUnequalSubject','.count.missing.percentage','.getParameterFixed'), .packages = c("MSstatsMC"), .combine='resultsAsLists', .options.snow = opts, .multicombine=TRUE, .init=list(list(), list(), list(), list())) %dopar% {
