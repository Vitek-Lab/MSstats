#' Given a processedData() set, this function should provide a bunch of fun plots of it.
#'
#' Ideally, I would fill in starting here the various plots, but I am lazy and tired.
#'
#' @param data  The result from processData()
#' @param type  Which type of plot to produce? (ProfilePlot, QCPlot, or ConditionPlot)
#' @param featureName  There are a few available features to plot, I know Transition is one.
#' @param ylimUp  Provide a upper boundary
#' @param ylimDown  or a lower boundary
#' @param scale  Choose a scale for the plot?
#' @param interval  Use confidence intervals?
#' @param x.axis.size  Choose a size for the axes?
#' @param y.axis.size  Also the y-axis.
#' @param text.size  Choose a size for the text?
#' @param text.angle  And an angle, stuff like this should be in arglist(...) IMO
#' @param legend.size  Choose a legend size?
#' @param dot.size.profile How big of a glyph for the conditions?
#' @param width  And a width for the plot
#' @param height and a height for it too!
#' @param which.Protein plot them all?
#' @param originalPlot  Plot the original data?
#' @param summaryPlot  and the summary of the data?
#' @param save_condition_plot_result  ok, wtf?  We have camelCase, camel.dot.Case, dot.args, and now this too?
#' @param address  Who wants it!?
#' @return plots!
#'
#' I am thinking to split each of these into its own function rather than being
#'   forced to come here for plotting, then just have this function call the
#'   others.
#' @export
dataProcessPlots <- function(data=data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
  ...
  ## I don't want to be jerk or anything, but this function was (well, still is) a complete shitshow.
  up_type <- toupper(type)
  plot_data <- list(
    "profile" = NULL,
    "qc" = NULL,
    "condition" = NULL)
  switchret <- switch(
    up_type,
    "PROFILEPLOT" = {
      plot_data[[up_type]] <- do_profile_plot(
        data=data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
        ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
        y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
        dot.size.profile=2, dot.size.condition=3, width=10,
        height=10, which.Protein="all", originalPlot=TRUE,
    ...
    "CONDITIONPLOT" = {
      plot_data[[up_type]] <- do_condition_plot(
        data=data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
        ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
        y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
        dot.size.profile=2, dot.size.condition=3, width=10,
    ...
    {
      message("An unknown plot type was requested, so you get all of them!")
      plot_data[["PROFILEPLOT"]] <- do_profile_plot(
        data=data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
        ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
        y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
        dot.size.profile=2, dot.size.condition=3, width=10,
      ...
      plot_data[["QCPLOT"]] <- do_qc_plot(
        ...
        height=10, which.Protein="all", originalPlot=TRUE,
        summaryPlot=TRUE, save_condition_plot_result=FALSE, address="")
      plot_data[["CONDITIONPLOT"]] <- do_condition_plot(
        data=data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
        ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
        y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
        dot.size.profile=2, dot.size.condition=3, width=10,
...
do_profile_plot <- function(data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
                            ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
                            y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
                            dot.size.profile=2, dot.size.condition=3, width=10,
                            height=10, which.Protein="all", originalPlot=TRUE,
  ...
  datafeature$RUN <- factor(datafeature$RUN, levels=unique(datafeature$RUN),
  ...
  if (sum(is.element(colnames(datarun), "RUN")) != 0) {
    datamat <- reshape2::dcast(Protein ~ RUN, data=datarun, value.var="LogIntensities", keep=TRUE)
    datarun <- reshape2::melt(datamat, id.vars=c("Protein"))
    ...
    for (i in 1:nlevels(datafeature$PROTEIN)) {
      ...
      if (haverun) {
        ...
        if (any(is.element(colnames(sub), "censored"))) {
          ...
          ptempall <- ptempall+geom_point(data=final,
...
do_qc_plot <- function(data, type="QCPLOT", featureName="Transition", ylimUp=FALSE,
                       ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
                       y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
                       dot.size.profile=2, dot.size.condition=3, width=10,
                       height=10, which.Protein="all", originalPlot=TRUE,
  ...
  if (address != FALSE) {
    ...
    message(paste0("Writing plots to: ", finalfile))
    pdf(finalfile, width=width, height=height)
  }
  ...
  if (which.Protein != "allonly") {
    if (which.Protein != "all") {
      ...
      if (is.numeric(which.Protein)) {
        ...
        if (length(levels(datafeature[["PROTEIN"]])) < max(which.Protein)) {
          dev.off()
          stop(paste("Please check your selection of proteins. There are ",
                     length(levels(datafeature[["PROTEIN"]])),
                     " proteins in this dataset.", sep=" "))
        }
      }
    ...
    for (i in 1:nlevels(datafeature[["PROTEIN"]])) {
      ...
      print(ptemp)
      protein_name <- unique(sub[["PROTEIN"]])
      plotlist[[protein_name]] <- ptemp
      message(paste("Drew the Quality Contol plot(boxplot) for ", protein_name, "(", i,
                    " of ", length(unique(datafeature[["PROTEIN"]])), ")"))
    } # end-loop iterating over every "PROTEIN" feature
  }
...
do_condition_plot <- function(data, type="ProfilePlot", featureName="Transition", ylimUp=FALSE,
                              ylimDown=FALSE, scale=FALSE, interval="CI", x.axis.size=10,
                              y.axis.size=10, text.size=4, text.angle=0, legend.size=7,
                              dot.size.profile=2, dot.size.condition=3, width=10,
                              height=10, which.Protein="all", originalPlot=TRUE,
  ...
  if (save_condition_plot_result) {
    ...
    if (address != FALSE) {
      ...
      finalfile <- paste0(address, "ConditionPlot_value.csv")
