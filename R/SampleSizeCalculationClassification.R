...
designSampleSizeClassification <- function(data, n = 5, step = 20, noise = 0.0, iter = 5){
  ...
  processout <- rbind(processout,as.matrix(c(" ", " ", "MSstats - designSampleSizeClassification function"," "), ncol=1))
  ...
  # check n
  if (!(n>0) & (is.integer(n))) {
    processout <- rbind(processout,c(paste("The required input - n : was not a positive integer. - stop")))
    write.table(processout, file=finalfile, row.names=FALSE)
  ...
  if (!(step>0) & (is.integer(step))) {
    processout <- rbind(processout,c(paste("The required input - step : was not a positive integer. - stop")))
    write.table(processout, file=finalfile, row.names=FALSE)
  ...
  # check noise
  if (!is.numeric(noise)) {
    processout <- rbind(processout,c(paste("The required input - noise : was not a numeric value - stop")))
    write.table(processout, file=finalfile, row.names=FALSE)
  ...
  if (!(iter>0) & (is.integer(iter))) {
    processout <- rbind(processout,c(paste("The required input - iter : was not a positive integer. - stop")))
    write.table(processout, file=finalfile, row.names=FALSE)
    
    stop("Please set iter as a positive integer.")
  }
  
  processout <- rbind(processout, c(paste("n = ", n, sep="")))
  processout <- rbind(processout, c(paste("step = ", step, sep="")))
  processout <- rbind(processout, c(paste("noise = ", noise, sep="")))
  processout <- rbind(processout, c(paste("iter = ", iter, sep="")))
  ...
  rownames(meanPA)<- paste("tra", train_size, sep="")
  colnames(meanPA)<- paste("val", validation_size, sep="")
  rownames(varPA)<- paste("tra", train_size, sep="") 
  colnames(varPA)<- paste("val", validation_size, sep="")
  ...
  return(list(optTrainSize = opt, meanPA = meanPA, varPA = varPA))
}
...
#' @import lme4
#' @return \emph{mu} is the mean abundance matrix of each protein in each phenotype group; \emph{sigma} is the sd matrix of each protein in each phenotype group.
.estimateVar <-  function(data) {
  ...
  return(list(mu=MeanAbun[1:count, ], sigma=Var[1:count, ]))
}
...
designSampleSizeClassificationPlots <- function(data) {
  ...
  p <- ggplot(data = plotdata , aes(x= as.numeric(as.character(Training_size)), y= mean, group = Valid_size, colour = Valid_size)) +
    geom_point() +
    geom_line() +
