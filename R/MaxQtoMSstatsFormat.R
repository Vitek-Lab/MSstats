...
## annotation : annotation.txt - Raw.file, Condition, BioReplicate, Run,
## (IsotopeLabelType)
## proteinGroups : proteinGroups.txt . if proteinGroups=NULL, use 'Proteins'. if
## not, use proteinGroups information for matching Protein group ID

## proteinID : Proteins or proteinGroupID
## useUniquePeptide : remove peptides that are assigned for more than one
## proteins. We assume to use unique peptide for each protein.
## summaryforMultipleRows : max or sum - when there are multiple measurements
## for certain feature and certain fun, use highest or sum of all.
## fewMeasurements : if 1 or 2 measurements across runs per feature, 'remove'
## will remove those featuares. It can affected for unequal variance analysis.
## remove_m_sequencepeptides : remove the peptides including 'M' sequence
## experiment : "DDA" or "SILAC"

#' @export
MaxQtoMSstatsFormat <- function(evidence, annotation, proteinGroups, proteinID="Proteins",
...
### possible to have some combination in Protein.group.IDs in infile, such as
### 64;1274;1155;1273 instead of 64, 1274.. separately.
...
### for example, protein.IDs in proteinGroups.txt are P05204;O00479. but, two
### 'proteins in evidence.txt, such as P05204;O00479, and P05204.
  ...
  if (experiment == "SILAC") {
    infile <- infile[c("uniqueProteins", "Protein.group.IDs", "Sequence",
                       "Modified.sequence", "Charge", "Raw.file", "Intensity.L",
                       "Intensity.H", "Retention.time", "id")]
...
cast_maxquant_to_wide_glf <- function(d_long, aggregateFun=aggregateFun) {
  data_w <- dcast(Proteins + Modified.sequence + Charge ~ Raw.file,
                  data=d_long, value.var="Intensity",
                  fun.aggregate=aggregateFun, keep=TRUE)
  ## keep=TRUE : will keep the data.frame value as 1 even though there is no
  ## values for certain feature and certain run.
  ## when there is completely missing in certain feature and certain run, '1'
  ## will be filled. Therefore put NA instead of 1.
  data_w[data_w == 1] <- NA
  return(data_w)
}
...
cast_maxquant_to_wide_silac <- function(d_long, aggregateFun=aggregateFun) {
  ...
  ##data_w = dcast( Proteins + Modified.sequence + Charge + IsotopeLabelType ~
  ##Raw.file, data=d_long, value.var='Intensity')
...
remove_feature_with_few <- function(x) {
  count_measure <- apply(x[, !(colnames(x) %in%
                               c("Proteins", "Modified.sequence", "Charge"))], 1,
