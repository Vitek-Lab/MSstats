...
#' @export
ProgenesistoMSstatsFormat <- function(input, annotation, useUniquePeptide=TRUE,
...
##############################
  ...
  if (!all(required.column %in% input[2, ])) {
    ...
    stop(paste("**",
               paste(required.column[missedInput], collapse = ", "),
  ...
  if (!all(required.annotation %in% colnames(annotation))) {
    ...
    stop(paste("**",
               paste(required.annotation[missedAnnotation], collapse = ", "),
