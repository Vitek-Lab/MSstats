...
groupComparison <- function(contrast.matrix=contrast.matrix, data=data) {
  ...
  for (i in 1:nlevels(rqall$Protein)) {
    ...
    if (data$SummaryMethod=="logOfSum") {
      ...
      ## fit the model
      temp <- try(.fit.model.single(contrast.matrix, sub, TechReplicate,
                                    singleSubject, repeated, origGroup, processout), silent=TRUE)
    }
  ...
  if (length(unique(data2$GROUP_ORIGINAL)) == 1) {
    ...
    for (k in 1:nrow(contrast.matrix)) {
      ...
      if (any(levels(origGroup)[contrast.matrix.sub != 0] == unique(data2$GROUP_ORIGINAL))) {
        processout <- rbind(processout, "*** error : results of Protein ",
