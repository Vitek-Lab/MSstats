...
groupComparisonPlots <- function(data=data,
  ...
  if (logBase.pvalue != 2 & logBase.pvalue != 10) {
    processout <- rbind(
      processout,
      c("ERROR : (-) Logarithm transformation for adjusted p-values : log2 or log10 only - stop"))
    write.table(processout, file=finalfile, row.names=FALSE)
    ...
    if (is.numeric(which.Comparison)) {
      ...
      if (length(levels(data$Label)) < max(which.Comparison)) {
        stop(paste("Please check your selection of comparisons. There are ",
                   length(levels(data$Label)), " comparisons in this result.", sep=" "))
      }
    }
...
#######################
  if (type == "HEATMAP") {
    ...
    for(j in 1:numheatmap) {
      ...
      par(oma=c(3, 0, 0, 4))
      heatmap.2(
        as.matrix(tempobj), col=my.colors, Rowv=FALSE, Colv=FALSE, dendrogram="none",
        breaks=my.breaks, trace="none", na.color="grey",
        ## missing value will be grey
        cexCol=(x.axis.size / 10), cexRow=(y.axis.size / 10), # assign text.size as option
        key=FALSE, lhei=c(0.1, 0.9), lwid=c(0.1, 0.9))
    }
...
#######################
  if (type == "VOLCANOPLOT") {
    ...
    for (i in 1:nlevels(data$Label)) {
      ...
      subtemp[!is.na(subtemp$issue) & subtemp$issue == "oneConditionMissing", "Protein"] <- paste0(
        "*", subtemp[!is.na(subtemp$issue) & subtemp$issue == "oneConditionMissing", "Protein"])

      ## Plotting
      if (logBase.pvalue == 2) {
        ptemp <- ggplot(aes_string(x='logFC', y='log2adjp', color='colgroup', label='Protein'),
                        data=subtemp) +
      ...
      if (logBase.pvalue == 10) {
        ptemp <- ggplot(aes_string(x='logFC', y='log10adjp', color='colgroup', label='Protein'),
                        data=subtemp) +
          geom_point(size=dot.size) +
          scale_colour_manual(values=c("gray65", "blue", "red"),
                              limits=c("black", "blue", "red"),
      ...
      if (ProteinName) {
        if (length(unique(subtemp$colgroup)) == 1 & any(unique(subtemp$colgroup) == 'black')) {
          message(paste0("The volcano plot for ",
          ...
          ptemp <- ptemp +
            geom_text_repel(data=subtemp[subtemp$colgroup != "black", ],
                            aes(label=Protein), size=text.size, col='black')
        }
      }
        ...
        if (logBase.pvalue == 10) {
          ...
          pfinal <- ptemp +
            geom_line(data=sigcut,
                      ...
                      colour="darkgrey", size=0.6, show.legend=TRUE) +
            scale_linetype_manual(values=c('twodash'=6),
                                  labels=c(paste0("Adj p-value cutoff (", sig, ")"))) +
            guides(colour=guide_legend(override.aes=list(linetype=0)),
                   linetype=guide_legend())
...
#######################
  if (type == "COMPARISONPLOT") {
      ...
      if (is.numeric(which.Protein)) {
        ...
        if (length(levels(datatemp$Protein)) < max(which.Protein)) {
          stop(paste0("Please check your selection of proteins. There are ",
                      length(levels(datatemp$Protein)),
    ...
    for (i in 1:nlevels(datatemp$Protein)) {
      ...
      if (colnames(data)[3] == "log2FC") {
        ptemp <- ptemp +
          scale_y_continuous("Log2-Fold Change", limits=c(y.limdown, y.limup))
      }
      if (colnames(data)[3] == "log10FC") {
        ptemp <- ptemp +
          scale_y_continuous("Log10-Fold Change", limits=c(y.limdown, y.limup))
      }
      print(ptemp)
